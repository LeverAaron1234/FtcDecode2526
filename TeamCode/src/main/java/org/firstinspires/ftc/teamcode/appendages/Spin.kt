...
class Spin(hardwareMap: HardwareMap) {
    ...
    fun odomUpdate(drive: MecanumDrive, isRedGoal: Boolean, turretOffset:Int, leftPressed: Boolean, rightPressed: Boolean, lock: Boolean): MutableList<Double?> {
        ...
        val targetX = if (isRedGoal) -57.0 else -72.0     //Why is the goal in a different position, is this the problem or a symptom
...
// Tunables (good candidates to move into DriveConstants for live tuning)
        val deadband = 0.5                        // deg: inside this we just hold
